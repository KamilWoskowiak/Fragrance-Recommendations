...
This parallelism brings the crawl time for \~20 k pages down to **≈2 hours**, vs ≈20 h single‑threaded.

### 2.2 Normalisation & Feature Engineering

| Dimension          | Technique                                                                                                              |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| **Accords**        | Collapsed to 11 macro groups (e.g. *Woody & Earthy, Citrus & Fresh*) → reduces sparsity and improves interpretability. |
...
applied to:<br>
  • `gender_score` (scale **‑2 ↔ +2**) using<br>
  • `priceValue_score` in the same way. |
\| **Rarity Boost** | Note matrix → TF‑IDF → re‑scaled accord columns so rare notes punch above frequency‑based averages. |
...
$$
$$
w_{sim}=0.35 \cdot (1-\text{diversity})
$$
...
while len(selected) < k:
    pick = argmax_i λ*R[i] - (1-λ)*max_j cos(V[i], V[j])
    selected.append(pick)
```

### 3.4 Output Annotations

* **gender\_label**: bucketed score → “Feminine / Unisex / Masculine”.
* **price\_value\_label**: “Very Overpriced / Fair Price / Great Value”.
* **dominant\_accords**: top accords > 30 %.
...
| Hosting | **AWS Lambda** (Python 3.12 image) behind API Gateway; cold‑start < 400 ms after packaging with all deps. |                                                                                                                |
...
│   └── dataProcessing/
│       └── DataProcessing.ipynb  # cleaning, TF‑IDF, feature eng
│
